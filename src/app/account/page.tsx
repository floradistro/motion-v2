...
function SkeletonCard() {
  return (
    <div className="rounded-2xl border border-white/[0.04] bg-surface p-6 animate-pulse">
...
function SkeletonOrderRow() {
  return (
    <div className="rounded-2xl border border-white/[0.04] bg-surface p-5 animate-pulse">
        ...
        <div className="max-w-4xl mx-auto">
              ...
              {editing ? (
                <div className="flex flex-col gap-3">
                  <div className="flex gap-3">
                    <input
                      ...
                      className="bg-surface border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm font-light text-white placeholder:text-muted/30 focus:outline-none focus:border-cyan/30 w-full"
                    />
                    <input
                      type="text"
                      value={editLast}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={saving}
                      className="text-[12px] tracking-[0.15em] uppercase bg-cyan text-black px-5 py-2 rounded-xl hover:bg-cyan/80 transition-colors disabled:opacity-50"
                    >
                      {saving ? "Saving..." : "Save"}
                    </button>
                    <button
                      onClick={() => setEditing(false)}
                      className="text-[12px] tracking-[0.15em] uppercase text-muted/50 hover:text-white px-5 py-2 rounded-xl transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <>
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    {memberSince && (
                      <span className="text-[11px] text-muted/30 font-light">
                        Member since{" "}
                  ...
                  <button
                    ...
                    className="text-[12px] text-muted/40 hover:text-cyan font-light mt-2 transition-colors"
                  >
                    Edit profile
                  </button>
                </>
          ...
          >
            {enrichedLoading || ordersLoading ? (
              ...
              <>
                {/* Total Orders */}
                <div className="rounded-2xl border border-white/[0.04] bg-surface p-6">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p className="text-[11px] tracking-[0.25em] uppercase text-muted/80">
                      Orders
                    </p>
                  </div>
                ...
                <div className="rounded-2xl border border-white/[0.04] bg-surface p-6">
                  ...
                  <p className="text-2xl font-extralight tracking-tight number-glow text-cyan">
                    {rewardPoints}
                  </p>
                </div>
          ...
          {!ordersLoading && activeOrders.length > 0 && (
            <motion.div
              ...
              className="mb-12"
            >
              <h2 className="text-[11px] tracking-[0.25em] uppercase text-muted/80 mb-4">
                Active Shipments
              </h2>
                      ...
                      {order.tracking_number && (
                        <div className="text-right">
                          {order.tracking_url ? (
                            ...
                            <p className="text-[12px] text-muted/50 font-light">
                              {order.carrier ? `${order.carrier}: ` : ""}
                              {order.tracking_number}
                            </p>
                          )}
                          {order.shipped_at && (
                            <p className="text-[11px] text-muted/30 font-light mt-0.5">
                              Shipped{" "}
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              <h2 className="text-[11px] tracking-[0.25em] uppercase text-muted/80">
                Order History
              </h2>
              {orders.length > 0 && (
                <div className="flex gap-2 flex-wrap">
                  {FILTER_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={`text-[11px] tracking-[0.12em] uppercase px-3.5 py-1.5 rounded-full transition-colors ${
                        filter === opt
                          ? "bg-cyan/15 text-cyan border border-cyan/20"
                          : "text-muted/40 hover:text-white border border-white/[0.06] hover:border-white/[0.12]"
                      }`}
                    >
                      {opt}
                    </button>
            ...
            {ordersLoading ? (
              ...
              <div className="space-y-3">
                {filteredOrders.map((order) => (
                  <div
                    key={order.id}
                    className="rounded-2xl border border-white/[0.04] bg-surface hover:border-white/[0.08] transition-colors group"
                  >
                    <Link
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <span
                          className={`text-[11px] tracking-[0.15em] uppercase px-3 py-1 rounded-full hidden sm:inline-block ${
                            STATUS_COLORS[order.status] || "text-muted/50 bg-white/5"
                          }`}
                        >
                          {order.status}
                        </span>
                    ...
                    {order.items?.some((i) => i.product_id) && (
                      <div className="px-5 pb-4 -mt-1">
                        <button
                          ...
                          className="text-[11px] tracking-[0.12em] uppercase text-muted/40 hover:text-cyan transition-colors disabled:opacity-50"
                        >
          ...
          {!enrichedLoading && !ordersLoading && rewardPoints > 0 && (
            ...
            >
              ...
              <div className="rounded-2xl border border-white/[0.04] bg-surface p-8">
                <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                  <div className="flex-1">
                    <p className="text-4xl font-extralight tracking-tight number-glow text-cyan">
                      {rewardPoints.toLocaleString()}
                    </p>
                    <p className="text-[12px] text-muted/50 font-light mt-1">
                      Reward Points
                    </p>
                  </div>
                  {enriched?.loyalty_tier && (
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-muted/30 font-light mt-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              {orders.length > 0 && orders[0]?.items?.some((i) => i.product_id) ? (
                <button
                  ...
                  className="rounded-2xl border border-white/[0.04] bg-surface p-6 hover:border-white/[0.08] transition-colors text-left group"
                >
                  <svg className="w-5 h-5 text-muted/30 group-hover:text-cyan transition-colors mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182" />
                  </svg>
                  <p className="text-sm font-light text-white group-hover:text-cyan transition-colors">
                    {reordering === orders[0].id ? "Adding..." : "Reorder Last"}
                  </p>
                  <p className="text-[11px] text-muted/30 font-light mt-1">
                    #{orders[0].order_number}
                  </p>
                </button>
              ) : (
                <div className="rounded-2xl border border-white/[0.04] bg-surface p-6 opacity-40">
                  <svg className="w-5 h-5 text-muted/30 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182" />
                  </svg>
                  <p className="text-sm font-light text-muted/50">Reorder Last</p>
                  <p className="text-[11px] text-muted/20 font-light mt-1">No orders yet</p>
                </div>
              )}
              ...
              <Link
                href="/shop"
                className="rounded-2xl border border-white/[0.04] bg-surface p-6 hover:border-white/[0.08] transition-colors group"
              >
                <svg className="w-5 h-5 text-muted/30 group-hover:text-cyan transition-colors mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
